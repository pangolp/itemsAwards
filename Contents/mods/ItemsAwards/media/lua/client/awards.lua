    ...
    for key, value in pairs(itemsAwards) do
        if (number == value.Number) then
            if (countZombieKill >= value.zkills) then
                ...
                -- Aquí pasamos el tipo de item y el mensaje para la UI
                if AddAwardMessageToUI then
                    AddAwardMessageToUI(value.Item, awardMessage)
                end

            else
                local needMoreKillsMessage = string.format(getText("IGUI_YouNeedMoreKills"), number, value.zkills)
                if Awards.Options.showMessageInChat then
                    attacker:Say(needMoreKillsMessage)
                else
                    attacker:setHaloNote(needMoreKillsMessage, 255, 0, 0, 300)
                end

                if AddLoserMessageToUI then
                    AddLoserMessageToUI(needMoreKillsMessage)
                end
            end
