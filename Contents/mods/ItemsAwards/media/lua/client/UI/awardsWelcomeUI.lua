...
function AwardsWelcomeUI:create()
    ...
    self.awardsList.itemheight = 32 -- Alto suficiente para íconos grandes
...
function AwardsWelcomeUI:addAwardMessage(itemFullType, message)
    local icon, name, count = nil, message, ""
    if itemFullType then
        local item = InventoryItemFactory.CreateItem(itemFullType)
        if item then
            icon = item:getTex()
            name = item:getDisplayName()
        end
    end
    local qty = string.match(message, "x(%d+)")
...
function AddAwardMessageToUI(itemFullType, message)
    if awardsWelcomeWindow then
        awardsWelcomeWindow:addAwardMessage(itemFullType, message)
    end
end
